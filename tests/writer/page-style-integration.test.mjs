...
test("UNO worker updates the current Writer page style through direct properties", () => {
  ...
  assert.match(worker, /OPENWORD_WRITER_PAGE_STYLES\.updatesFor\(command\)/);
...
test("visible header and footer controls use semantic commands", () => {
  const home = read("src/components/WriterHomeBar.svelte");
  assert.match(home, /aria-label="Header and footer"/);
